...
var ARRAY_SIZE_Y = 3;
var ARRAY_SIZE_Z = 4;
var NavMeshTestScene = class extends cc.Scene {
  constructor() {
    super();
...
var NavMeshBaseTestDemo = class extends NavMeshTestScene {
  _title = "NavMesh Test";
  _camera = null;
  ...
  onTouchesMoved(touches, event) {
    if (touches.length > 0 && this._camera) {
      ...
      this._camera.lookAt(cc.math.vec3(0, 0, 0), cc.math.vec3(0, 1, 0));
      if (delta.x * delta.x + delta.y + delta.y > 16) this._needMoveAgents = false;
    }
  }
  onTouchesEnded(touches, event) {
    if (!this._needMoveAgents) return;
    if (touches.length > 0) {
  ...
  createAgent(pos) {
    ...
    this._agents[this._agents.length] = {
      "agent": agent,
      "animate": animate
    };
  }
    ...
    for (var index = 0; index < this._agents.length; index++) {
      var callback = function (agent, totalTimeAfterMove) {
        ...
        if (agent.isOnOffMeshLink()) {
          ...
          var dir = cc.math.vec3(linkdata.endPosition.x - linkdata.startPosition.x, linkdata.endPosition.y - linkdata.startPosition.y, linkdata.endPosition.z - linkdata.startPosition.z);
          dir.y = 0.0;
          dir.normalize();
          var axes;
...
var NavMeshBasicTestDemo = class extends NavMeshBaseTestDemo {
...
var NavMeshAdvanceTestDemo = class extends NavMeshBaseTestDemo {
  _title = "Navigation Mesh Test";
...
// Flow control
//
var arrayOfNavMeshTest = [NavMeshBasicTestDemo, NavMeshAdvanceTestDemo];
...
var previousNavMeshTest = function () {
  NavMeshTestIdx--;
  if (NavMeshTestIdx < 0) NavMeshTestIdx += arrayOfNavMeshTest.length;
  return new arrayOfNavMeshTest[NavMeshTestIdx]();
};
