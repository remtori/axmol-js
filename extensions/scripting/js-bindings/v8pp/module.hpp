...
public:
    ...
    explicit module(v8::Isolate* isolate) : isolate_(isolate), obj_(v8::ObjectTemplate::New(isolate)) {}

    /// Create new module in the specified V8 isolate for existing ObjectTemplate
    explicit module(v8::Isolate* isolate, v8::Local<v8::ObjectTemplate> obj) : isolate_(isolate), obj_(obj) {}
    ...
    module& submodule(std::string_view name, v8pp::module& m) { return value(name, m.obj_); }

    /// Set wrapped C++ class in the module with specified name
    template <typename T, typename Traits = v8pp::object_ptr_traits>
    module& class_(std::string_view name, v8pp::class_<T, Traits>& cl)
    {
        v8::HandleScope scope(isolate_);

        cl.js_function_template()->SetClassName(v8pp::to_v8(isolate_, name));
        return value(name, cl.js_function_template());
    }

    /// Set a C++ function in the module with specified name
    template <typename Function, typename Traits = v8pp::object_ptr_traits>
    ...
    module& property(char const* name, GetFunction&& get, SetFunction&& set = {})
    {
        ...
        static_assert(detail::is_callable<Setter>::value || std::same_as<Setter, detail::none>,
        ...
        v8::AccessorNameSetterCallback setter =
            property_type::is_readonly ? nullptr : property_type::template set<Traits>;
    ...
    v8::Local<v8::Object> new_instance() { return obj_->NewInstance(isolate_->GetCurrentContext()).ToLocalChecked(); }
