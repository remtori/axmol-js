...
struct call_from_v8_traits
{
    ...
    using arg_converter = typename std::conditional_t<is_wrapped_class<std::remove_cv_t<U>>::value,
...
inline constexpr bool is_direct_args =
    CallTraits::arg_count == (Offset + 1) &&
...
inline constexpr bool is_first_arg_isolate =
    CallTraits::arg_count != (Offset + 0) && std::same_as<typename CallTraits::template arg_type<Offset>, v8::Isolate*>;

template <typename Traits, typename F, typename CallTraits, size_t... Indices, typename... ObjArg>
decltype(auto) call_from_v8_impl(F&& func,
...
decltype(auto) call_from_v8(F&& func, v8::FunctionCallbackInfo<v8::Value> const& args, ObjArg&... obj)
{
    ...
    else
    {
        ...
        {
            throw std::runtime_error("Argument count does not match function definition. Expected " +
        ...
        if constexpr (with_isolate)
        {
            return (call_from_v8_impl<Traits>(std::forward<F>(func), args, call_traits{}, indices{}, obj...,
                                              args.GetIsolate()));
        }
        else
        {
