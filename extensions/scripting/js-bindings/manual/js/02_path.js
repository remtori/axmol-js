/// From https://github.com/nodejs/node/blob/ef2e0848ba366ca278d936f1f793d455188e792e/lib/path.js

"use strict";

((window) => {
    const { primordials, internals, ops } = window.__bootstrap;
    const { ax } = window.__bindings;
    ...
    const isWindows = ops.op_is_windows();

    function processCwd() {
        return ax.FileUtils.getInstance().getDefaultResourceRootPath();
    }
                ...
                if (lastSlash === i - 1 || dots === 1) {
                    ...
                    if (
                        ...
                        StringPrototypeCharCodeAt(res, res.length - 1) !==
                            CHAR_DOT ||
                        StringPrototypeCharCodeAt(res, res.length - 2) !==
                            CHAR_DOT
                    ) {
                        if (res.length > 2) {
                            const lastSlashIndex =
                                res.length - lastSegmentLength - 1;
                            if (lastSlashIndex === -1) {
                                res = "";
                                lastSegmentLength = 0;
                            } else {
                                res = StringPrototypeSlice(
                                    res,
                                    0,
                                    lastSlashIndex
                                );
    ...
    const win32 = {
        ...
        resolve(...args) {
            ...
            for (let i = args.length - 1; i >= -1; i--) {
                ...
                if (i >= 0) {
                    ...
                    path = process.env[`=${resolvedDevice}`] || processCwd();
                    ...
                    if (
                        path === undefined ||
                        (StringPrototypeToLowerCase(
                            StringPrototypeSlice(path, 0, 2)
                        ) !== StringPrototypeToLowerCase(resolvedDevice) &&
                            StringPrototypeCharCodeAt(path, 2) ===
                                CHAR_BACKWARD_SLASH)
                    ) {
                        path = `${resolvedDevice}\\`;
                    }
                ...
                if (len === 1) {
                    ...
                    if (isPathSeparator(StringPrototypeCharCodeAt(path, 1))) {
                        ...
                        if (j < len && j !== last) {
                            const firstPart = StringPrototypeSlice(
                                path,
                                last,
                                j
                            );
                            ...
                            while (
                                j < len &&
                                isPathSeparator(
                                    StringPrototypeCharCodeAt(path, j)
                                )
                            ) {
                                j++;
                            }
            ...
            // but handle relative paths to be safe (might happen when processCwd()
            // fails)
                        ...
                        if (j < len && j !== last) {
                            ...
                            if (j === len || j !== last) {
                                if (firstPart === "." || firstPart === "?") {
                                    ...
                                    return `\\\\${firstPart}\\${StringPrototypeSlice(
                                        path,
                                        last
                                    )}\\`;
                                } else {
            ...
            if (
                ...
                do {
                    if (
                        index === len - 1 ||
                        isPathSeparator(
                            StringPrototypeCharCodeAt(path, index + 1)
                        )
                    ) {
                        return `.\\${tail}`;
                    }
                } while (
                    (index = StringPrototypeIndexOf(path, ":", index + 1)) !==
                    -1
                );
            }
            if (device === undefined) {
        ...
        join(...args) {
            ...
            if (isPathSeparator(StringPrototypeCharCodeAt(firstPart, 0))) {
                ...
                if (
                    ...
                    if (firstLen > 2) {
                        if (
                            isPathSeparator(
                                StringPrototypeCharCodeAt(firstPart, 2)
                            )
                        )
                            ++slashCount;
                        else {
            ...
            if (needsReplace) {
                ...
                while (
                    ...
                    isPathSeparator(
                        StringPrototypeCharCodeAt(joined, slashCount)
                    )
                ) {
                    slashCount++;
                }
        ...
        relative(from, to) {
            ...
            if (
                fromOrig.length !== from.length ||
                toOrig.length !== to.length
            ) {
                ...
                if (i === 0) {
                    ...
                    if (toLen > length) {
                        return ArrayPrototypeJoin(
                            ArrayPrototypeSlice(toSplit, i),
                            "\\"
                        );
                    }
                    if (fromLen > length) {
                        return (
                            StringPrototypeRepeat("..\\", fromLen - 1 - i) +
                            ".."
                        );
                    }
                    return "";
                ...
                return (
                    StringPrototypeRepeat("..\\", fromLen - i) +
            ...
            while (
                ...
                StringPrototypeCharCodeAt(from, fromStart) ===
                    CHAR_BACKWARD_SLASH
            ) {
                fromStart++;
            }
            ...
            while (
                fromEnd - 1 > fromStart &&
                StringPrototypeCharCodeAt(from, fromEnd - 1) ===
                    CHAR_BACKWARD_SLASH
            ) {
                fromEnd--;
            }
            ...
            if (
                StringPrototypeCharCodeAt(toOrig, toStart) ===
                CHAR_BACKWARD_SLASH
            )
                ++toStart;
        ...
        toNamespacedPath(path) {
            ...
            if (
                StringPrototypeCharCodeAt(resolvedPath, 0) ===
                CHAR_BACKWARD_SLASH
            ) {
                // Possible UNC root
                if (
                    StringPrototypeCharCodeAt(resolvedPath, 1) ===
                    CHAR_BACKWARD_SLASH
                ) {
                    const code = StringPrototypeCharCodeAt(resolvedPath, 2);
                    if (code !== CHAR_QUESTION_MARK && code !== CHAR_DOT) {
                        // Matched non-long UNC root, convert the path to a long UNC path
                        return `\\\\?\\UNC\\${StringPrototypeSlice(
                            resolvedPath,
                            2
                        )}`;
                    }
                ...
                StringPrototypeCharCodeAt(resolvedPath, 2) ===
                    CHAR_BACKWARD_SLASH
            ) {
                ...
                for (let i = path.length - 1; i >= start; --i) {
                    ...
                    if (isPathSeparator(code)) {
                        ...
                        if (extIdx >= 0) {
                            ...
                            if (
                                code ===
                                StringPrototypeCharCodeAt(suffix, extIdx)
                            ) {
                                if (--extIdx === -1) {
        ...
        parse(path) {
            ...
            if (end !== -1) {
                if (
                    ...
                    ret.base = ret.name = StringPrototypeSlice(
                        path,
                        startPart,
                        end
                    );
    ...
    const posixCwd = (() => {
        if (isWindows) {
            ...
            const regexp = /\\/g;
            return () => {
                const cwd = StringPrototypeReplace(processCwd(), regexp, "/");
                return StringPrototypeSlice(
                    cwd,
                    StringPrototypeIndexOf(cwd, "/")
                );
            };
        }
    ...
    const posix = {
        ...
        resolve(...args) {
            ...
            // handle relative paths to be safe (might happen when processCwd() fails)
    ...
    internals.path = isWindows ? win32 : posix;
})(globalThis);
