/// From https://github.com/denoland/deno/tree/main/ext/console
...
((window) => {
    const { core, internals, primordials, ops } = window.__bootstrap;
    const {
        isAnyArrayBuffer,
        isArgumentsObject,
        isArrayBuffer,
        ...
        if (ctx.customInspect) {
            if (
                ...
                if (
                    ...
                    const depth =
                        ctx.depth === null ? null : ctx.depth - recurseTimes;
    ...
    function getKeys(value, showHidden) {
        ...
        if (showHidden) {
            ...
            if (symbols.length !== 0) {
                ...
                ArrayPrototypePushApply(
                    keys,
                    ArrayPrototypeFilter(symbols, filter)
                );
            }
        }
    ...
    function formatSet(value, ctx, _ignored, recurseTimes) {
        ...
        for (let i = 0; i < len; i++) {
            ArrayPrototypePush(
                output,
                formatValue(ctx, values[i], recurseTimes)
            );
        }
        if (remaining > 0) {
            ArrayPrototypePush(
    ...
    const AGGREGATE_ERROR_NOT_EMPTY_LINE_PATTERN = new SafeRegExp(
        /^(?!\s*$)/gm
    );
        ...
        if (
            ...
            for (let i = 0; i < outputLength; i += columns) {
                ...
                for (; j < max - 1; j++) {
                    ...
                    // others.
                    const padding =
                        maxLineLength[j - i] + output[j].length - dataLen[j];
    ...
    function parseCssColor(colorString) {
        ...
        // deno-fmt-ignore
        const smallHashMatch = StringPrototypeMatch(
            colorString,
            SMALL_HASH_PATTERN
        );
    ...
    function cssToAnsi(css, prevCss = null) {
        ...
        if (
            !colorEquals(css.textDecorationColor, prevCss.textDecorationColor)
        ) {
    ...
    class Console {
        ...
        table = (data = undefined, properties = undefined) => {
            ...
            const indexKey =
                isSetObject || isMapObject ? "(iter idx)" : "(idx)";
                    ...
                    for (let i = 0; i < keys.length; ++i) {
                        ...
                        if (!primitive && ReflectHas(valueObj, k)) {
                            if (!ReflectHas(objectValues, k)) {
                                objectValues[k] = ArrayPrototypeFill(
                                    new Array(numRows),
                                    ""
                                );
                            }
    ...
    function createFilteredInspectProxy({ object, keys, evaluate }) {
        ...
        function getDescendantPropertyDescriptor(object, key) {
            let propertyDescriptor = ReflectGetOwnPropertyDescriptor(
                object,
                key
            );
    ...
    ObjectDefineProperty(window, "console", {
