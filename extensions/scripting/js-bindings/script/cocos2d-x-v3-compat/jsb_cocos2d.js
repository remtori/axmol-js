cc.ENGINE_VERSION = "Axmol-js 2.6.2";
...
cc.ccBlendToAxmol = [
    ...
    cc.CONSTANT_ALPHA,
    cc.SRC_ALPHA_SATURATE,
    cc.ONE_MINUS_CONSTANT_ALPHA,
    cc.BLEND_COLOR,
];
...
cc.ccTexParamsToAxmol = {
    [gl.NEAREST]: 0,
    [gl.NEAREST_MIPMAP_NEAREST]: 1,
    [gl.NEAREST_MIPMAP_LINEAR]: 2,
    [gl.LINEAR]: 3,
    [gl.LINEAR_MIPMAP_LINEAR]: 4,
    [gl.LINEAR_MIPMAP_NEAREST]: 5,
    [gl.REPEAT]: 0,
    [gl.MIRRORED_REPEAT]: 1,
    [gl.CLAMP_TO_EDGE]: 2,
};
...
cc.UNIFORM_COS_TIME_S = "AX_CosTime";
cc.UNIFORM_COSTIME_S = "AX_CosTime";
cc.UNIFORM_SIN_TIME_S = "AX_SinTime";
cc.UNIFORM_SINTIME_S = "AX_SinTime";
cc.UNIFORM_PMATRIX_S = "AX_PMatrix";
cc.UNIFORM_MVMATRIX_S = "AX_MVMatrix";
cc.UNIFORM_MVPMATRIX_S = "AX_MVPMatrix";
cc.UNIFORM_P_MATRIX_S = "AX_PMatrix";
cc.UNIFORM_MV_MATRIX_S = "AX_MVMatrix";
cc.UNIFORM_MVP_MATRIX_S = "AX_MVPMatrix";
cc.UNIFORM_RANDOM01_S = "AX_Random01";
cc.UNIFORM_SAMPLER_S = "AX_Texture0";
cc.UNIFORM_ALPHA_TEST_VALUE_S = "AX_AlphaValue";
...
cc.Loader = class {
    initWith (resources, selector, target) {
        if (selector) {
...
cc.EventListener.create = function (argObj) {
    ...
    if (listenerType === cc.EventListener.TOUCH_ONE_BY_ONE) {
        ...
        listener = cc.EventListenerCustom.create(
            argObj.eventName,
            argObj.callback
        );
...
cc.eventManager.addCustomListener = cc.eventManager.addCustomEventListener;
cc.eventManager.pauseTarget = cc.eventManager.pauseEventListenersForTarget;
cc.eventManager.removeListener = cc.eventManager.removeEventListener;
cc.eventManager.removeListeners = cc.eventManager.removeEventListenersForType;
cc.eventManager.removeAllListeners = cc.eventManager.removeAllEventListeners;
...
cc.EventMouse.prototype.getLocationX = function () {
    return this.getLocation().x;
};

cc.EventMouse.prototype.getLocationY = function () {
    return this.getLocation().y;
};

cc.EventMouse.prototype.getDeltaX = function () {
    return this.getDelta().x;
};

cc.EventMouse.prototype.getDeltaY = function () {
    return this.getDelta().y;
};

cc.EventMouse.prototype.getButton = cc.EventMouse.prototype.getMouseButton;
...
cc.Director.prototype.setDepthTest = function(depthTest) {
    this.getRenderer().setDepthTest(depthTest);
}
...
cc.DrawNode = class extends cc._DrawNode {
    _drawColor = cc.color(255, 255, 255, 255);
    _lineWidth = 1;
    _convex = false;

    setIsConvex(isConvex) {
    ...
    drawRect(origin, destination, fillColor, lineWidth, lineColor) {
        ...
        if (fillColor) {
            super.drawSolidRect(origin, destination, fillColor, lineWidth, lineColor);
        } else {
            super.drawRect(origin, destination, lineColor, lineWidth);
        }
    }
    ...
    drawCircle(center, radius, angle, segments, drawLineToCenter, lineWidth, color) {
        ...
        super.drawCircle(center, radius, angle, segments, drawLineToCenter, color, lineWidth);
    }
    ...
    drawQuadBezier(origin, control, destination, segments, lineWidth, color) {
        ...
        super.drawQuadBezier(origin, control, destination, segments, color, lineWidth);
    }
    ...
    drawCubicBezier(origin, control1, control2, destination, segments, lineWidth, color) {
        ...
        super.drawCubicBezier(origin, control1, control2, destination, segments, color, lineWidth);
    }
    ...
    drawCatmullRom(points, segments, lineWidth, color) {
        ...
        super.drawCatmullRom(points, segments, color, lineWidth);
    }

    drawCardinalSpline(points, tension, segments, lineWidth, color) {
        lineWidth = lineWidth || this._lineWidth;
        color = color || this._drawColor;
        if (color.a == null)
            color.a = 255;

        super.drawCardinalSpline(points, tension, segments, color, lineWidth);
    }

    drawDot(pos, radius, color) {
        color = color || this._drawColor;
        super.drawDot(pos, radius, color);
    }
    ...
    drawSegment(from, to, lineWidth, color) {
        ...
        super.drawSegment(from, to, lineWidth, color);
    }

    drawPoly(verts, fillColor, borderWidth, borderColor) {
        borderColor = borderColor || this._drawColor;
        if (fillColor) {
            super.drawPolygon(verts, fillColor, borderWidth, borderColor, this._isConvex);
        } else {
            super.drawPolygon(verts, borderWidth, borderColor, this._isConvex);
        }
    }
};
...
var templateSetBlendFunc = function (src, dst) {
    ...
    blendf.src = cc.ccBlendToAxmol.indexOf(blendf.src);
    blendf.dst = cc.ccBlendToAxmol.indexOf(blendf.dst);
...
cc.callFunc = (callback, definedTarget, param) => {
    return cc.CallFunc.create(target => callback.call(definedTarget || target, target, param));
}
...
cc.rectApplyAffineTransform = function (rect, anAffineTransform) {
    ...
    var bottomLeft = cc._pointApplyAffineTransform(
        left,
        bottom,
        anAffineTransform
    );
    var bottomRight = cc._pointApplyAffineTransform(
        right,
        bottom,
        anAffineTransform
    );
...
const UserDataSymbol = Symbol('NodeUserData');
cc.Node.prototype.setUserData = function (data) {
    this[UserDataSymbol] = data;
};
cc.Node.prototype.getUserData = function () {
    return this[UserDataSymbol];
};

const UserObjectSymbol = Symbol('NodeUserObject');
cc.Node.prototype.setUserObject = function (data) {
    this[UserObjectSymbol] = data;
};
cc.Node.prototype.getUserObject = function () {
    return this[UserObjectSymbol];
};
...
cc.Node.prototype._scheduleUpdate = cc.Node.prototype.scheduleUpdate;
cc.Node.prototype.scheduleUpdate = function () {
    this._scheduleUpdate(this.update, 0);
};

cc.Node.prototype.scheduleUpdateWithPriority = function (priority) {
    this._scheduleUpdate(this.update, priority);
};

cc.Node.prototype._unscheduleUpdate = cc.Node.prototype.unscheduleUpdate;
cc.Node.prototype.unscheduleUpdate = function () {
    this._unscheduleUpdate(this.update);
};
...
cc.Texture2D.prototype.setTexParameters = function (
    ...
    this._setTexParameters(
        cc.ccTexParamsToAxmol[minFilter],
        cc.ccTexParamsToAxmol[magFilter],
        cc.ccTexParamsToAxmol[wrapS],
        cc.ccTexParamsToAxmol[wrapT]);
};

cc.Texture2D.prototype.handleLoadedTexture = function (premultiplied) {};

cc.Menu.prototype.alignItemsInColumns = function (...args) {
    return this.alignItemsInColumnsWithArray(args);
};

cc.Menu.prototype.alignItemsInRows = function (...args) {
    return this.alignItemsInRowsWithArray(args);
};
...
if (cc.sys.os === cc.sys.OS_ANDROID && cc.audioEngine) {
    ...
    cc.audioEngine.playMusic = function () {
        var args = arguments;
        var searchPaths = cc.fileUtils.getSearchPaths();
        var path = args[0];
        searchPaths.some(function (item) {
            var temp = item + "/" + path;
            var exists = cc.fileUtils.isFileExist(temp);
            if (exists) {
                path = temp;
                return true;
            }
...
_p._scheduleUpdateForTarget = _p.scheduleUpdateForTarget;
_p.scheduleUpdateForTarget = function (target, priority = 0, paused = false) {
    if (target instanceof cc.Node) {
        this._scheduleUpdateForTarget(target, target.update, priority, paused);
    } else {
        this._scheduleUpdateForTarget(target, priority, paused);
    }
};
_p._unscheduleUpdateForTarget = _p.unscheduleUpdateForTarget;
_p.unscheduleUpdateForTarget = function (target) {
    if (target instanceof cc.Node) {
        this._unscheduleUpdateForTarget(target, target.update);
    } else {
        this._unscheduleUpdateForTarget(target);
    }
};
